...
class SNEList:
    ...
    **Keywords**:
     -bw = the bandwidth of the kde to calculate. Default is 2.5 degrees for angular data and 2.5 cm for thickness data. 
    ...
    def getKDE(self,variable,**kwds):
        ...
        if "thick" in variable.lower():
            if self.thickKDE is None or recalc:
                ...
                bw = kwds.get("bw",0.025)
                
                #build grid
                self.thickGrid = np.linspace(gmin,gmax,res)

                #evaluate KDE
                self.thickKDE = scipy.stats.gaussian_kde(self.thickness,bw_method=bw)(self.thickGrid)
